...
export default function WhatsAppCTA() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 font-sans flex flex-col items-end">
      {/* Action Window */}
      {isOpen && (
        <div className="mb-4 w-[320px] max-w-[calc(100vw-2rem)] rounded-2xl glass-panel shadow-blueGlow border border-white/10 p-5 overflow-hidden animate-float">
          ...
          <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-[10px] text-khuvo-slate">
            <span className="flex items-center gap-1"><ShieldCheck className="w-3 h-3 text-khuvo-cyan" /> Certified Experts</span>
            <span className="flex items-center gap-1"><Zap className="w-3 h-3 text-khuvo-cyan" /> Direct Dispatch</span>
          </div>
        </div>
      )}
